...
col_gldas_geo_25km_tz_ind_group = \
...
for iyr in range(len(daysofyear)):
    ...
    for idc in range(daysofyear[iyr]):
        ...
        for itm in range(len(ind_timezone)):
            lst_gldas_utc_am_geo_1day[:, col_gldas_geo_25km_tz_ind_group[itm]] = \
                lst_gldas_geo_3day[:, col_gldas_geo_25km_tz_ind_group[itm],
                (lst_am_utct_day[itm] + 1) * 8 + lst_am_gldas_ind[itm] // 3]
            lst_gldas_utc_pm_geo_1day[:, col_gldas_geo_25km_tz_ind_group[itm]] = \
                lst_gldas_geo_3day[:, col_gldas_geo_25km_tz_ind_group[itm],
                (lst_pm_utct_day[itm] + 1) * 8 + lst_pm_gldas_ind[itm] // 3]
            sm_gldas_utc_am_geo_1day[:, col_gldas_geo_25km_tz_ind_group[itm]] = \
                sm_gldas_geo_3day[:, col_gldas_geo_25km_tz_ind_group[itm],
                (sm_am_utct_day[itm] + 1) * 8 + sm_am_gldas_ind[itm] // 3]
            sm_gldas_utc_pm_geo_1day[:, col_gldas_geo_25km_tz_ind_group[itm]] = \
                sm_gldas_geo_3day[:, col_gldas_geo_25km_tz_ind_group[itm],
...
for ife in range(len(gldas_files)):
    ...
    if ife != len(gldas_files)-1:
        ...
        for x in range(len(varname_list_fe1)):
...
for imo in range(len(monthnum)):#range(len(monthnum)):
    ...
    for ipx in range(len(ltdr_ndvi)):
        ...
        # Output Order: coefficient, intercept, r2, rmse
        # Fit AM data
